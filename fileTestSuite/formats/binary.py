...
from .. import FilesSubset, FTSMetadata, OnthArg, Onthology, Parameter, ParsedMetadataJSONLikeT
from .core import IFormat, Serializer
...
class Headered(Serializer):
	...
	@classmethod
	def serialize(cls, self: FTSMetadata, stream: BytesIO) -> None:
...
class VersionS(Serializer):
	...
	@classmethod
	def serialize(cls, self: Version, stream: BytesIO) -> None:
...
class HeaderS(Serializer):
	...
	def parse(cls, stream: BytesIO) -> "Header":
		...
		return Header(sigRaw.decode("utf8"), VersionS.parse(stream))

	@classmethod
	def serialize(cls, self: Header, stream: BytesIO) -> None:
...
class StrS(Serializer):
	...
	@classmethod
	def serialize(cls, self: str, stream: BytesIO) -> None:
